...
class TestDistroVersionController(object):
    ...
    def test_distro_should_list_unique_versions(self, session):
        ...
        Binary('ceph-1.0.1.deb', p, ref='main', sha1="head", distro='ubuntu', distro_version='trusty', arch='i386')
        session.commit()
        result = session.app.get('/binaries/ceph/main/head/ubuntu/trusty/')
...
class TestDistroController(object):
    ...
    def test_list_a_distro_version(self, session):
        ...
        Binary('ceph-1.0.0.deb', p, ref='main', sha1="head", distro='ubuntu', distro_version='trusty', arch='i386')
        session.commit()
        result = session.app.get('/binaries/ceph/main/head/ubuntu/')
    ...
    def test_list_a_distinct_distro_version(self, session):
        ...
        result = session.app.get(
            '/binaries/ceph/main/head/debian/',
            expect_errors=True)
    ...
    def test_list_a_distro_version_not_found(self, session):
        ...
        session.commit()
        result = session.app.get('/binaries/ceph/main/head/ubuntu/', expect_errors=True)
        assert result.status_int == 404
    ...
    def test_single_distro_should_have_one_item(self, session):
        p = Project('ceph')
        Binary('ceph-1.0.0.deb', p, ref='main', sha1="head", distro='ubuntu', distro_version='12.04', arch='i386')
        session.commit()
        result = session.app.get('/binaries/ceph/main/head/ubuntu/')
    ...
    def test_refs_should_not_pollute_others(self, session):
        p = Project('ceph')
        Binary('ceph-1.0.0.deb', p, ref='main', sha1="head", distro='ubuntu', distro_version='precise', arch='i386')
        Binary('ceph-1.0.0.deb', p, ref='jewel', sha1="head", distro='ubuntu', distro_version='xenial', arch='arm64')
        session.commit()
        result = session.app.head('/binaries/ceph/main/head/ubuntu/xenial/arm64/', expect_errors=True)
        assert result.status_int == 404
    ...
    def test_distros_should_not_pollute_others(self, session):
        ...
        session.commit()
        result = session.app.head('/binaries/ceph/main/head/debian/', expect_errors=True)
        assert result.status_int == 404
